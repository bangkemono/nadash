...
<div class="flex min-h-screen justify-center items-center bg-gray-900">
    {#if visible}
        <div in:fly={{ y: 50, duration: 1000, delay: 0 }} class="flex flex-col justify-center bg-gray-800 w-96 p-8 rounded-2xl shadow-2xl border border-gray-700 space-y-6">
            ...
            <form method="POST" class="w-full space-y-4" use:enhance={handleLogin}>
                {#if form?.error}
                    <div in:fade={{ duration: 300, delay: 0 }} class="p-3 bg-red-500/20 border border-red-500/50 rounded-lg text-red-200 text-xs text-center">
                        {form.error}
                    </div>
                {:else if form?.success}
                    <div in:fade={{ duration: 500, delay: 0 }} class="p-3 bg-green-500/20 border border-green-500/50 rounded-lg text-green-200 text-xs text-center" out:fade={{ duration: 1000, delay: 0 }}>
